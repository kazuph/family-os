...
function makeHarness(pdfChunks = ["%PDF-1.4"], closePdf = true) {
  ...
  let page = {
    ...
    goto: async (url: string) => {
      ...
      request?.({
        ...
        respond: ({body, headers}: {body: string; headers?: Record<string, string>}) => {
          ...
          exportDocument = body;
          exportDocumentCsp = headers?.["Content-Security-Policy"];
        },
      });
      request?.({
        ...
        frame: () => mainFrame,
        continue: () => { blobRequestContinued = true; },
      });
      request?.({
    ...
    mainFrame: () => mainFrame,
    emulateMediaType: async (value: string) => { mediaType = value; },
    evaluate: (fn: (...args: never[]) => unknown, ...args: unknown[]) => {
      if (typeof fn === "string" || fn.toString().includes("__workshopExportSanitizeHtml") ||
          fn.toString().includes("scrollWidth")) {
        return evaluate(false, fn, ...args);
      }
      if (fn.toString().includes("__workshopExportModulePromise")) {
        clientInitialized = true;
    ...
    screenshot: async (options?: {
      ...
      if (!options?.clip && options?.captureBeyondViewport === undefined) {
        return new Uint8Array([137, 80, 78, 71]);
      }
      let {type = "png", clip, captureBeyondViewport} = options;
      screenshotType = type;
      screenshotClip = clip;
