...
全体検査の入力は `30eadf26`。後続の承認一覧・接続削除修正は下記の追加検査で確認する。
ローカル検査は以下のとおり。PRの独立レビューと本番反映確認は別の出荷手順として残る。

| 検査 | 結果・原ログ |
| --- | --- |
| 全体 `pnpm test` | PASS: `job-1788789842936-3086-2941` |
| 全体 `pnpm lint`（型検査含む） | PASS: `job-1788789729407-3086-2940` |
| 全体build | PASS: `job-1788790016109-3086-2943` |
...
独立レビュー後、移動専用接続を元workspaceの通常APIから開ける経路、
元workspace削除で未割り当て接続が消える経路、取り戻しで自動承認の無効化が失われる経路を修正した。
接続の削除はbinding解除だけでなく既存のfacet削除処理へ委譲する。
新しいGadget単位のSpawner作成APIはhost内のIDを使い、既存workspace単位APIの任意envは維持する。
移動先gadget IDとhost接続IDの衝突、未割り当て接続の保持と削除、取り戻し後の承認待ちを
実workerdで確認した（15件PASS: `job-1788789639196-3086-2938`）。
自動承認を無効にした後のカタログ表示も追加確認した（11件PASS: `job-1788789686948-3086-2939`）。
取り戻し後のlocal scoped承認を通常一覧へ集約し、接続削除時は作成元の参照と承認設定も削除する。
有効/無効の取り戻し、一覧からの無効化、削除後の一覧除去の追加検査12件PASS
（`job-1788790312948-3086-2946`）。
以前からある `open-gadget-rpc.test.ts` の4件のskipはこの変更では増減しておらず、
全体テスト成功はそれらの検証完了を意味しない。
